const weave = require('./solution_2');
...
test('weave can combine two queues: length of queue one is greater than queue two', () => {
  const one = new Queue();
  one.add(3);
  one.add(2);
  one.add(1);
...
test('weave can combine two queues: length of queue one is lower than queue two', () => {
  const one = new Queue();
  one.add('Hi');
